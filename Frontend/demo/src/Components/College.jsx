import React, { useState, useEffect } from 'react';
import { URL } from "../config";
import axios from 'axios';
import "../CSS/University.css"



const College = () => {
    const [colleges, setColleges] = useState([]);
    // const [normal, setNormal] = useState(false);
    const [rankflag, setRankflag] = useState(false);
    const [rankcolleges, setRankColleges] = useState([]);

    const hardcodedImageURL="data:image/jpeg;base64,"

    const getData = async () => {
        try {
            await axios.get(`${URL}/college/`).then((response) => {
                console.log(response.data);
                let arr = response.data;
                setColleges(response.data);
            }).catch();
        } catch (e) {
            console.log("Error occured", e);
        }
  }
    useEffect(() => {
        getData();
    }, []);
    const getCityColleges = async () => {
        try {
            await axios.get(`${URL}/college/city`).then((response) => {
                console.log(response.data);
                let arr = response.data;
                setColleges(response.data);
                //console.log(arr.uniId.uniName);
            }).catch();
        } catch (e) {
            console.log("Error occured", e);
        }
    }
    const getRankColleges = async () => {
        try {
            setRankflag(true)
            await axios.get(`${URL}/college/rank`).then((response) => {
                console.log(response.data);
                let arr = response.data;
                setRankColleges(response.data);
                console.log(arr[0].clgId.clgName);
            }).catch();
        } catch (e) {
            console.log("Error occured", e);
        }
    }

  return (
      <div>
              <nav className="navbar navbar-expand-lg navbar-dark bg-dark">
              <div className="container">
                          <button
          className="navbar-toggler mr-5"  style={{padding:"10px",marginLeft:"50px",marginRight:"20px"}}
          type="button"
          data-bs-toggle="collapse"
          data-bs-target="#navbarNav"
          aria-controls="navbarNav"
          aria-expanded="false"
          aria-label="Toggle navigation"
                 onClick={getCityColleges} >Sort by city</button>
                   <button
          className="navbar-toggler" style={{padding:"10px",marginLeft:"50px",marginRight:"20px"}}
          type="button"
          data-bs-toggle="collapse"
          data-bs-target="#navbarNav"
          aria-controls="navbarNav"
          aria-expanded="false"
          aria-label="Toggle navigation"
           onClick={getRankColleges}       >Sort by Rank</button>
                  <a href="/collegereg"> <button
          className="navbar-toggler" style={{padding:"10px",marginLeft:"50px",marginRight:"0px",align:"right"}}
          type="button"
          data-bs-toggle="collapse"
          data-bs-target="#navbarNav"
          aria-controls="navbarNav"
          aria-expanded="false"
          aria-label="Toggle navigation"
                  >Add Institute</button></a>
              </div>
                  </nav>

      <div style={{marginTop:"10px",marginBottom:"20px"}}>
    <h2 align="center">List of Colleges</h2>
              <div className="container-fluid mt-5 " >{

                  !rankflag?<>
                  <div className="row text-center" >
            
                      {

                          colleges.map((curElem) => {
                
                              const { clgImage, clgName, clgDescription, uniId, clgAddress, clgCity, clgWebsite, clgId } = curElem;
                              let uni = curElem.uniId;
                              return (
                                  <div className="col-10 col-md-4 mt-5" key={clgId} >
                                      <div className="card p-2" style={{margin:"20px"}} >
                                          <div className="custom-card" style={{padding:"20px"}} >
                                              <div className="custom-image img" align="center"> <img src={hardcodedImageURL} className="rounded" width="155" /> </div>
                                              <div className="ml-3 w-100 " style={{ alignContent: "" }}>
                                                  <h3 className="mb-0 mt-0 textLeft"> <b>Name :</b> {clgName}</h3>
                                                  <br></br>
                                                  <ul className="custom-ul">
                                                      <li><b>University : </b>{curElem.uniId.uniName}</li>
                                                      <li><span className="text-left"> <b>College Description :</b> {clgDescription}</span></li>
                                                      <li><span className="text-left"><b> Address :</b>{clgAddress}</span></li>
                                                      <li><b>City : {clgCity}, {curElem.clgState}</b></li>
                                                      <li><b>Website : </b><a href={curElem.clgWebsite}>Click here</a></li>

                                                  </ul>
                                              </div>
                                          </div>
                                      </div>
                                  </div>
                              )
                          })
                      }
            
                      </div>
                  </>: <div className="row text-center" >
            
                      {

                          rankcolleges.map((curElem) => {
                
                            //   const { clgImage, clgName, clgDescription, uniId, clgAddress, clgCity, clgWebsite, clgId } = curElem;
                            //   let uni = curElem.uniId;
                              return (
                                  <div className="col-10 col-md-4 mt-5" key={curElem.rnkId} >
                                      <div className="card p-2" >
                                          <div className="d-flex align-items-center" >
                                              <div className="image"> <img src={hardcodedImageURL} className="rounded" width="155" /> </div>
                                              <div className="ml-3 w-100 " style={{ alignContent: "" }}>
                                                  <h3 className="mb-0 mt-0 textLeft">{curElem.clgId.clgName}</h3>
                                                  <br></br>
                                                  <ul>
                                                      <li><b>NIRF Rank : </b>{curElem.nirfRank}</li>
                                                      <li><span className="text-left"> <b>College Description :</b> {curElem.clgId.clgDescription}</span></li>
                                                      <li><span className="text-left"><b> Address :</b>{curElem.clgId.clgAddress}</span></li>
                                                      <li><b>City : {curElem.clgId.clgCity}, {curElem.clgId.clgState}</b></li>
                                                      <li><b>Website : </b><a href={curElem.clgId.clgWebsite}>Click here</a></li>

                                                  </ul>
                                              </div>
                                          </div>
                                      </div>
                                  </div>
                              )
                          })
                      }
            
                      </div>}
    </div>
</div>

    </div>
  )
}

export default College;
