...
const TestimonialCard = ({ name, feedback,feedid,rating}) => {
  return (
    ...
    <div className="testimonial-card">
      {feedid % 4 == 0 ? <> <img src={image3} alt={`${name}`} className="testimonial-image" /><h3>{name}</h3><p>{feedback}</p><p>{rating}/5</p></>
        : feedid % 4 == 1 ? <><img src={image4} alt={`${name}`} className="testimonial-image" /><h3>{name}</h3> <p>{feedback}</p><p>{rating}/5</p></>
